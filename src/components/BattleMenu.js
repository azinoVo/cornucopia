                ...
                <div className='user'>
                    ...
                    <p>Ultimate Points: {userBattleStats.ultimate}</p>
                    {
                        userAbilities.map(ability => {
                            return <button disabled={(ability.name === 'Ultimate: Release' && userBattleStats.ultimate === 0) ? true : false} onClick={() => battle(userBattleStats, currentEncounter, ability.name)} key={ability.name}>{ability.name}: {ability.description}</button>
                        })
                    }
                </div>
